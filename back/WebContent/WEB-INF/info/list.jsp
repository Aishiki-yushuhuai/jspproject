							...
							<c:if test="${!empty requestScope.pageHelper.data}">
								...
								<div style="margin : 0 auto ;width:450px;  height: 450px">
									<ul class="pagination pagination-lg">
										<li><a href="list.info?menuId=${requestScope.menu.id}">首页</a></li>
										<c:if test="${requestScope.pageHelper.page != 1}">
											<li><a href="list.info?menuId=${requestScope.menu.id}&page=${requestScope.pageHelper.page - 1}">&laquo;</a></li>
										</c:if>
										<c:forEach begin="1" end="${requestScope.pageHelper.pageCount}" var="i">
											<li><a href="list.info?menuId=${requestScope.menu.id}&page=${i}">${i}</a></li>
										</c:forEach>
										<c:if test="${requestScope.pageHelper.page != requestScope.pageHelper.pageCount}">
											<li><a href="list.info?menuId=${requestScope.menu.id}&page=${requestScope.pageHelper.page + 1}">&raquo;</a></li>
										</c:if>
										<li><a href="list.info?menuId=${requestScope.menu.id}&page=${requestScope.pageHelper.pageCount}">尾页</a></li>
									</ul>
								</div>
							</c:if>
							<c:if test="${empty requestScope.pageHelper.data}">
